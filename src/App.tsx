...
const OPENING = {
  title: "",
  body: "透過幾題簡單問答，靈膳魔導師將為您分析體質屬性，並依結果推薦專屬燕窩食用攻略與安永鮮物藥膳湯品搭配建議，打造您的每日靈膳儀式感。",
  cta: "開始測驗",
};
...
function sumScore(answers: Answer[]): number {
  return answers.filter(a => a.qid !== "Q7")
...
function profileByScore(total: number) {
  if (total >= 18) return { key: "靈氣充盈型", range: "18–21分", feature: "體質均衡，氣血旺盛，但需持續保養以維持亮麗光澤。", bird: "凰啼初盞", plan: "每週 2–3 次，早晨空腹食用『凰啼初盞』；可加入少量安永鮮物『花旗蔘氣養湯』或『四神健脾湯』作為氣養搭配。", quote: "持盈保泰，乃養生之道。氣滿，則神明自華。", soups: ["花旗蔘氣養湯","四神健脾湯"], time: "早晨" } as const;
  if (total >= 13) return { key: "氣血雙虛型", range: "13–17分", feature: "容易疲倦、頭暈，女性常有氣色不佳現象。", bird: "潤若朝霞", plan: "每週 3–4 次，晚餐後 1 小時食用『潤若朝霞』；可搭配安永鮮物『當歸鴨湯』或『藥膳排骨湯』溫補氣血。", quote: "氣為血帥，血為氣母；兩虛之時，燕窩溫養最宜。", soups: ["當歸鴨湯","藥膳排骨湯"], time: "晚餐後" } as const;
  if (total >= 9)  return { key: "陰虛火旺型", range: "9–12分", feature: "熬夜多、睡眠差、口乾舌燥，容易長痘或燥熱。", bird: "瑤光夜盞", plan: "每週 3 次，晚間食用『瑤光夜盞』；搭配安永鮮物『蓮子百合養心湯』或『枸杞雞湯』有助滋陰養心。", quote: "夜深時，讓月華與燕窩一同撫平你的燥與火。", soups: ["蓮子百合養心湯","枸杞雞湯"], time: "晚間" } as const;
  if (total >= 7)  return { key: "陽氣不足型", range: "7–8分", feature: "手腳冰冷、代謝低、容易疲倦。", bird: "凰啼初盞", plan: "每週 4 次，早晨溫熱食用『凰啼初盞』；搭配安永鮮物『薑母雞湯』或『人蔘燉雞湯』，助陽生氣。", quote: "陽升則生，陽退則衰。溫一盞晨光，重啟體內暖流。", soups: ["薑母雞湯","人蔘燉雞湯"], time: "早晨" } as const;
  return { key: "氣鬱體滯型", range: "6分以下", feature: "壓力大、睡不安、情緒起伏、消化不良。", bird: "潤若朝霞", plan: "每週 2–3 次，午後食用『潤若朝霞』；搭配安永鮮物『養肝舒氣湯』或『香附平衡湯』舒緩氣鬱。", quote: "情緒為氣之主。養氣即養心，心開則百脈順。", soups: ["養肝舒氣湯","香附平衡湯"], time: "午後" } as const;
}
...
function calcProgress(total: number, answered: number, isDone: boolean) {
  if (total <= 0) return 0;
...
function cleanLabel(s: string) {
  ...
  if (c >= 65 && c <= 68 && s.length > 1 && s[1] === " ") return s.slice(2);
  return s;
}

function buildActiveQuestions(): Question[] {
  const res: Question[] = QUESTIONS_7.map((q) => {
...
= ({ text, speed = 80, startKey, className, onDone }) => {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    setShowOptions(false);
    setPreparingIdx(0);
    const rot = setInterval(() => setPreparingIdx(i => (i + 1) % PREPARING_TEXTS.length), 1200);
    const t = setTimeout(() => { clearInterval(rot); setPhase("asking"); }, 3000);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setShowOptions(true), 150);
      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        <div className="rounded-2xl bg-white border border-gray-200 p-0 shadow-sm">
          <div className="p-3 md:p-4 border-b border-gray-200">
            ...
            <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden" role="progressbar" aria-valuenow={progressPct} aria-valuemin={0} aria-valuemax={100}>
              <div className="h-full bg-gray-800" style={{ width: progressPct + "%", transition: "width .3s ease" }} />
            </div>
          </div>

          <div ref={scrollRef} className="p-4 md:p-6 space-y-3 max-h[70vh] md:max-h-[70vh] overflow-y-auto no-scrollbar">
                  ...
                  <div className="space-y-4">
                    <div className="w-full flex justify-start">
                      <div className="max-w-[85%] rounded-2xl p-4 shadow-sm bg-white">
                        ...
                        <button className="px-4 py-2 rounded-xl shadow-sm border border-gray-200 hover:shadow transition" onClick={start}>🪄 開始測驗</button>
                      </div>
                    </div>
                  </div>
                </motion.div>
                      ...
                      <div className="w-full flex justify-start">
                        <div className="max-w-[85%] rounded-2xl p-4 shadow-sm bg-white">
                          <div className="mt-1 text-base font-medium">
                            <Typewriter text={questions[idx].title} startKey={`${idx}-${phase}`} speed={90} onDone={() => setQuestionTyped(true)} />
                          </div>
                        </div>
                      </div>
                      {showOptions && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {questions[idx].options.map((opt) => (
                            <button key={opt.key} className="px-4 py-2 rounded-xl shadow-sm border border-gray-200 hover:shadow transition text-left"
                    ...
                    <div className="max-w-[85%] rounded-2xl p-4 shadow-sm bg-white">
                      ...
                      <div className="mt-4 space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2 pt-1">
                          <a className="inline-flex items-center gap-2 px-4 py-2 rounded-xl shadow-sm border border-gray-200 hover:shadow transition"
                             href={lineDeepLink} target="_blank" rel="noreferrer">
